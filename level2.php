...
include "connect.php";
$sid = $_SESSION['pid'];
if (!isset($_SESSION['music_played2'])) {
    echo '<script>
    ...
    $_SESSION['music_played2'] = true;
}
$nqres = mysqli_query($conn, "SELECT count(*) from responses2 where sid='$sid';");
...
if (isset($_GET['qid'])) {
	...
	$query = "insert into responses2 (sid, qid, answer,marks) values ('$sid', $qid, '$response', $marks)";
					...
					<section class="card">
						...
						<div class="card-body">
							...
							if ($q <= 4) {
								...
								$ques = mysqli_query($conn, "SELECT * FROM words3 WHERE qid NOT IN (SELECT qid FROM responses2 WHERE sid='$sid') AND level=2 ORDER BY RAND() LIMIT 1;");
								...
								$userResponsesQuery = "SELECT qid, answer AS response, marks FROM responses2 WHERE sid='$sid'";
								...
								echo "<a href='level2.php?qid=$qid&op=$option1'><button type='submit' class='mb-1 mt-1 mr-1 btn btn-primary' class='box'>$option1</button></a>";
								echo "<a href='level2.php?qid=$qid&op=$option2'><button type='submit' class='mb-1 mt-1 mr-1 btn btn-primary' class='box'>$option2</button></a>";
								echo "<a href='level2.php?qid=$qid&op=$option3'><button type='submit' class='mb-1 mt-1 mr-1 btn btn-primary' class='box'>$option3</button></a>";
								echo "<a href='level2.php?qid=$qid&op=$option4'><button type='submit' class='mb-1 mt-1 mr-1 btn btn-primary' class='box'>$option4</button></a>";

								echo "</div>";
							} else {
								echo "<h3 style='color:red;' align='center'>YOUR LEVEL-2 KBC HAS BEEN COMPLETED!</h3>";
